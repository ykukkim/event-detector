...
import numpy as np
import sys
sys.path.append("/Users/YKK/Documents/GitHub/event-detector/btkmac")
import btk
import keras
...
def extract_kinematics(leg, filename_in):
    ...
    if rate != 300:
        print('Check the Sampling Frequency')
        return
    ...
    if brk:
        print("No kinematics in %s!" % (filename_in,))
        return
...
def process(filename_in, filename_out):
    ...
    events = {}
    events[("Foot Strike", "Left")] = neural_method(XR, modelHS)
    events[("Foot Strike", "Right")] = neural_method(XL, modelHS)
    events[("Foot Off", "Left")] = neural_method(XR, modelFO)
    events[("Foot Off", "Right")] = neural_method(XL, modelFO)
